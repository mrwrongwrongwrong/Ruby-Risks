  ...
  methods: {
    ...
    addAI() {
      if (this.ifStart)
        return
      if (this.playerNum == 5) {
        alert('The number of players is 5, cannot add AIplayer.')
      } else {
        if (this.hasAI) {
          this.players.splice(this.players.indexOf('AIPlayer'), 1)
          this.gameResult += 'Delete AIPlayer Success.\n'
        } else {
          this.players.push('AIPlayer')
          this.gameResult += 'Add AIPlayer Success.\n'
        }
    ...
    submit() {
      ...
      if (!this.gameOver()) {
        ...
        if (this.boxIfOpened[this.ideaBoxIndex - 1]) {
          alert('The box ' + this.ideaBoxIndex + ' has been already opened, please choose another box.')
          return
        }
